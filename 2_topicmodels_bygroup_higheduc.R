...
# only highly educated
data <- subset(data, DE07 >= 6 & DE07 <= 8) # 6: abi, 7: uni, 8: doktor
# women
# data <- subset(data, DE03 == 2)
...
graphics <- lapply(wohntypenlvl, function(wohntypenx){
  ...
  write.xlsx(topic_prob_privat,
             file = paste0("analysis/out/private_topics_", wohntypenx, "_higheduc.xlsx"))
...
dev.copy(png,"analysis/out/private_topicmodels_wohntyp_higheduc.png", height = 800)
dev.off()
